...
main( int argc, char *argv[ ] )
{
    ...
    for( int n = 0;  n < MAXITERATIONS; n++ )
    {
        ...
        #pragma omp parallel for default(none) shared(Cities, Capitals) num_threads(NUMT)
        for( int i = 0; i < NUMCITIES; i++ )
        {
            int capitalnumber = -1;
            ...
            for( int k = 0; k < NUMCAPITALS; k++ )
            {
                ...
                if( dist < mindistance )
                {
                    Cities[i].capitalnumber = capitalnumber = k;
                    Cities[i].mindistance   = mindistance   = dist;
                }
            }
